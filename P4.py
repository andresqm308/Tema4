...
# ---
# 
# * Estudiantes: **Andres Quesada Molina B78911**,**Alejandro Zuñiga Perez B59097**, **Bryan Ramírez Ampie B35581**
# * Grupo: **9**
# 
# ---
...
def modulador_8PSK(bits, fc, mpp):
    # Aqui se va a definir el metodo para lograr simulaer el esquema de modulacion
    #8-PSK

    # 1. Se van a definir los parámetros para la señal de información (bits)
    N = len(bits) # Cantidad de bits  

    # 2. Se construyen las señales portadoras c(t)
    Tc = 1/fc   # Período [s]
    t_periodo = np.linspace(0, Tc, mpp) # Muestras por período

    # Se inician las señales portadoras 
    portadora_C = np.cos(2*np.pi*fc*t_periodo)
    portadora_S = np.sin(2*np.pi*fc*t_periodo)

    # 3. Se da inicio a las señales moduladas s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx = np.zeros(t_simulacion.shape)

    
    #4. Prueba inicial para la asignacion de los bits.
    h = np.sqrt(2)/2
    for i in range(0, N, 3): # Amplitudes
        
        # A1 y A2---portadora C y portadora S
        if bits[i]==1 and bits[i+1]==1 and bits[i+2]==1:
            senal_Tx[i*mpp : (i+1)*mpp]=1*portadora_C

        if bits[i]==1 and bits[i+1]==1 and bits[i+2]==0:
            senal_Tx[i*mpp : (i+1)*mpp]=h*portadora_C + h*portadora_S

        if bits[i]==0 and bits[i+1]==1 and bits[i+2]==0:
            senal_Tx[i*mpp : (i+1)*mpp]=1*portadora_S
     ...
     # Potencias oara cada una de las señales moduladas.
    P_m = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)

    return senal_Tx, P_m, portadora_C, portadora_S
...
def demodulador_8PSK(senal_Rx, portadora_C, portadora_S, mpp):
    '''Un método que simula un bloque demodulador
    ...
    :return: Los bits de la señal demodulada
    '''
    # Cantidad de muestras que existen en senal_Rx
    M = len(senal_Rx)
    ...
    # Vector para la señal demodulada
    senal_d1 = np.zeros(senal_Rx.shape)
    senal_d2 = np.zeros(senal_Rx.shape)

    # Pseudo-energía de un período de la portadora
    Es1 = np.sum(portadora_C * portadora_C)
    Es2 = np.sum(portadora_S*portadora_S)
    ...
    for i in range(N):
        # Producto interno de dos funciones
        producto_1 = senal_Rx[i*mpp : (i+1)*mpp] * portadora_C
        Ep1 = np.sum(producto_1) 
        producto_2 = senal_Rx[i*mpp : (i+1)*mpp] * portadora_S
        Ep2 =np.sum(producto_2)
        senal_d1[i*mpp : (i+1)*mpp] = producto_1
        senal_d2[i*mpp : (i+1)*mpp] = producto_2

        # Criterio de decisión por detección de energía

        if i % 3 == 0:
            if Ep1 >= (1+h)*Es1/2 and h*Es2/2 >= Ep2 >= -h*Es2/2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 1
            
            elif h*Es1/2 <= Ep1 <= (1+h)*Es1/2 and h*Es2/2 <= Ep2 <= (1+h)*Es2/2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 0

            elif -h*Es1/2 <= Ep1 <= h*Es1/2 and  Ep2 >= (1+h)*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 0

            elif -(h+1)*Es1/2 <= Ep1 <= -h*Es1/2 and h*Es2/2 <= Ep2 <= (1+h)*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 1

            elif -(h+1)*Es1/2 >= Ep1 and -h*Es2/2 <= Ep2 <= h*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 1

            elif -(h+1)*Es1/2 <= Ep1 <= -h*Es1/2 and -(h+1)*Es2/2 <= Ep2 <= -h*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 0

            elif h*Es1/2 >= Ep1 >= -h*Es1/2 and -(h+1)*Es2/2 >= Ep2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 0

            elif h*Es1/2 <= Ep1 <= (h+1)*Es1/2 and -(h+1)*Es2/2 <= Ep2 <= -h*Es2/2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 1

    return bits_Rx.astype(int), senal_d1, senal_d2
...
# 1. se importara y convertira la la imagen a trasmitir que se desea transmitir.
imagen_Tx = fuente_info('arenal.jpg')
dimensiones = imagen_Tx.shape

# 2. Se codifican los pixeles de la imagen.
bits_Tx = rgb_a_bit(imagen_Tx)

# 3. Se modula la cadena de bits utilizando el esquema 8-PSK
senal_Tx, Pm, portadora_C, portadora_S = modulador_8PSK(bits_Tx, fc, mpp)

# 4. Se va a simular y trnasmitir la señal modulada por el canal ruidoso.
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se demodulan los bits de la señal recibida del canal utilizando el esquema
#8-PSK
bits_Rx, senal_d1, senal_d2 = demodulador_8PSK(senal_Rx, portadora_C, portadora_S, mpp)


# 6. Se visualizara la imagen pasando de bits a RGB. 
imagen_Rx = bits_a_rgb(bits_Rx, dimensiones)
Fig = plt.figure(figsize=(10,6))

# Se calcula el tiempo de la simulacion.
print('Tiempo que dura la simulación: ', time.time() - inicio)

# 7. Se calcula el numero total de errores
E = sum(abs(bits_Tx - bits_Rx))
BER = E/len(bits_Tx)
print('Numero total de errores:{}'.format(E),', para un BER de {:0.4f}.'.format(BER))
